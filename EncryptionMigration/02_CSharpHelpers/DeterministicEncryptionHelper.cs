...
namespace Eoffice.Security
{
    ...
    /// 
    /// Keys are loaded from configuration (Web.config or App.config):
    ...
    public static class DeterministicEncryptionHelper
    {
        ...
        // Lazy load keys from configuration
        ...
        /// <summary>
        /// Loads encryption keys from configuration file (Web.config or App.config)
        /// </summary>
        private static void LoadKeysFromConfig()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(keyBase64) || string.IsNullOrEmpty(ivBase64))
                {
                    throw new ConfigurationErrorsException(
                        "Encryption keys not found in configuration. " +
            ...
            {
                throw new ConfigurationErrorsException(
                    "Invalid Base64 format for encryption keys in configuration.", ex);
            }
        }
